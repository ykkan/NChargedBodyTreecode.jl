...
function cluster_coord(bbox::BBox{T}; n=5) where {T}
    ...
    xcoords = (chebpts .+ 1.0) * (xmax - xmin)/2.0 .+ xmin
    ycoords = (chebpts .+ 1.0) * (ymax - ymin)/2.0 .+ ymin
    zcoords = (chebpts .+ 1.0) * (zmax - zmin)/2.0 .+ zmin
...
function make_cluster(particles::Particles, parindices::Vector{Int}, lo, hi, level; threshold) where {T}
    ...
    if npar <= threshold
        ...
        xcoords, ycoords, zcoords = cluster_coord(bbox)
        gamma_hat, mom_hat = cluster_weight(particles, parindices, lo, hi, bbox)
